// Generated by CoffeeScript 1.7.1
(function() {
  "use strict";
  var global, parseDecimals, parseNumber, plural, rubles, toFloat, words;

  words = [['', 'один', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь', 'девять', 'десять', 'одиннадцать', 'двенадцать', 'тринадцать', 'четырнадцать', 'пятнадцать', 'шестнадцать', 'семнадцать', 'восемнадцать', 'девятнадцать'], ['', '', 'двадцать', 'тридцать', 'сорок', 'пятьдесят', 'шестьдесят', 'семьдесят', 'восемьдесят', 'девяносто'], ['', 'сто', 'двести', 'триста', 'четыреста', 'пятьсот', 'шестьсот', 'семьсот', 'восемьсот', 'девятьсот']];

  toFloat = function(number) {
    return parseFloat(number);
  };

  plural = function(count, options) {
    var rest;
    if (options.length !== 3) {
      return false;
    }
    count = Math.abs(count) % 100;
    rest = count % 10;
    if (count > 10 && count < 20) {
      return options[2];
    }
    if (rest > 1 && rest < 5) {
      return options[1];
    }
    if (rest === 1) {
      return options[0];
    }
    return options[2];
  };

  parseNumber = function(number, count, options) {
    var first, numeral, second;
    numeral = '';
    if (options.intDenom == null) {
      options.intDenom = ['рубль', 'рубля', 'рублей'];
    }
    if (options.intDenomGender == null) {
      options.intDenomGender = 'masculine';
    }
    if (number.length === 3) {
      first = number.substr(0, 1);
      number = number.substr(1, 3);
      numeral = "" + words[2][first] + " ";
    }
    if (number < 20) {
      numeral += "" + words[0][toFloat(number)] + " ";
    } else {
      first = number.substr(0, 1);
      second = number.substr(1, 2);
      numeral += "" + words[1][first] + " " + words[0][second] + " ";
    }
    if (count === 0) {
      numeral += plural(number, options.intDenom);
      if (options.intDenomGender !== 'masculine') {
        numeral = numeral.replace('один ', 'одна ').replace('два ', 'две ');
      }
    } else if (count === 1) {
      if (numeral !== '  ') {
        numeral += plural(number, ['тысяча ', 'тысячи ', 'тысяч ']);
        numeral = numeral.replace('один ', 'одна ').replace('два ', 'две ');
      }
    } else if (count === 2) {
      if (numeral !== '  ') {
        numeral += plural(number, ['миллион ', 'миллиона ', 'миллионов ']);
      }
    } else if (count === 3) {
      numeral += plural(number, ['миллиард ', 'миллиарда ', 'миллиардов ']);
    }
    return numeral;
  };

  parseDecimals = function(number, options) {
    var text;
    if (options.decimalDenom == null) {
      options.decimalDenom = ['копейка', 'копейки', 'копеек'];
    }
    text = plural(number, options.decimalDenom);
    if (number === 0) {
      number = "00";
    } else if (number < 10) {
      number = "0" + number;
    }
    return " " + number + " " + text;
  };

  rubles = function(number, options) {
    var count, decimals, digit, length, numeral, parts, _ref, _ref1;
    if (options == null) {
      options = {};
    }
    if (!number || ((_ref = typeof number) !== 'number' && _ref !== 'string')) {
      return false;
    }
    if (typeof number === 'string') {
      number = toFloat(number.replace(',', '.'));
      if (isNaN(number)) {
        return false;
      }
    }
    if (number <= 0) {
      return false;
    }
    number = number.toFixed(2);
    if (number.indexOf('.') !== -1) {
      _ref1 = number.split('.'), number = _ref1[0], decimals = _ref1[1];
    }
    numeral = '';
    length = number.length - 1;
    parts = '';
    count = 0;
    while (length >= 0) {
      digit = number.substr(length, 1);
      parts = digit + parts;
      if ((parts.length === 3 || length === 0) && !isNaN(toFloat(parts))) {
        numeral = parseNumber(parts, count, options) + numeral;
        parts = '';
        count++;
      }
      length--;
    }
    numeral = numeral.replace(/\s+/g, ' ');
    if (decimals) {
      numeral += parseDecimals(toFloat(decimals), options);
    }
    return numeral;
  };

  global = typeof module !== "undefined" && module !== null ? exports : window;

  global.rubles = rubles;

}).call(this);
